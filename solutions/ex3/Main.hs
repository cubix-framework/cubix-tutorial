...
--
-- To that end, Cubix also provides another form of smart constructor, which cn also be used in pattern matching:
--
-- @
-- pattern Ident' :: (Ident :-<: fs, All HFunctor fs) => String -> Term fs IdentL
-- @
--
-- In the future `deriveAll` will generate these as well.
...
-- | PART 2a
-- Using `createSortInclusionType`, create sort injection nodes so that the generic
-- `Assign` node can replace `ImpAssign`, and generic `Block` an replace
-- `BlockStmt`. You will need to (1) create sort injections from
...
--
--
-- Also, if you want to see what's generated by Template Haskell
--
